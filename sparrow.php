...
class Sparrow {
     ...
     */
    public function execute($key = null, $expire = 0, $bindParams = array()) {
        if (!$this->db) {
            ...
            switch ($this->db_type) {
                case 'pdo':
                    try {
                        ...
                        // $testSql = "select * from author where name = :name";
                        $result = $this->db->prepare($this->sql);
                        // $result = $this->db->prepare($testSql);
                        ...
                        if (!$result) {
                            ...
                            if(count($this->bindParams) > 0){
                                $result->execute($this->bindParams);
                            }
                            else{
                                $result->execute();
                            }
                            ...
                            if(0 < count($this->bindParams))
                                $this->realSql = $this->generateRealSql($this->bindParams, $this->sql);
                            else
                                $this->realSql = $this->sql;
                        }
                    }
    ...
    /**
     * übernimmt die zu parsenden Parameter. Prepare Statement
     *
     * @param $bindParams
     *
     * @return $this
    ...
    * @return mixed
    */
    protected function generateRealSql($bindParams, $rawSql)
    {
       $keys = array();
       $values = array();
       ...
       if (count($bindParams) && is_string(key($bindParams))) {
           ...
           uksort($bindParams, function($k1, $k2) {
